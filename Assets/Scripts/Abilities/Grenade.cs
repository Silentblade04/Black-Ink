...
public class Grenade : MonoBehaviour
{
    ...
    [Tooltip("Which tile pool to use when highlighting this ability.")]
    public GridHighlighter.HighlightType highlightType = GridHighlighter.HighlightType.Action;
    ...
    private void Update()
    {
        ...
        if (Physics.Raycast(ray, out RaycastHit hitInfo, maxRange))
        {
            ...
            if (gridHighlighter != null)
            {
                gridHighlighter.ShowAreaAt(lastTargetPoint, diameter, highlightType);
            }
        }
        else
        {
            // If ray misses, hide highlights or optionally clear them - here we hide so player knows target is invalid
            if (gridHighlighter != null)
    ...
    public void ConfirmTargeting()
    {
        ...
        if (lastTargetPoint == Vector3.zero)
        {
...
//Grenade Process:
//Uses Active Raycast to highlight a 2block area
//upon confirmation deal damage to all targets in area
//resolve ability